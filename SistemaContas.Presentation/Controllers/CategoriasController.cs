...
{
    //Só usuário autenticado consegue acessar o controlador
    [Authorize]
    public class CategoriasController : Controller
    {
        ...
        public IActionResult Exclusao(Guid id)
        {
            try 
            {
                ...
                //verificando se a categoria foi encontrada e se ela pertence ao usuário autenticado
                if (categoria != null && categoria.IdUsuario == UsuarioAutenticado.Id)
                {
                    ...
                    {
                        TempData["MensagemAlerta"] = $"Não é possível excluir, pois existe(m) {qtdContas} conta(s) cadastrada(s) para esta categoria.";
                    }                
                }
            }
            catch (Exception e) 
        ...
        public IActionResult RelatorioExcel()
        {
            try
            {
                ...
                var categoriaRepository = new CategoriaRepository();
                var categoria = categoriaRepository.GetByUsuario(UsuarioAutenticado.Id);

                //gerar um relatório excel com as categorias
                var categoriasReportService = new CategoriasReportService();
                var relatorio = categoriasReportService.GerarRelatorioExcel(categoria);

                //Download do relatório
                //O navegador entende o tipo de arquivo pelo mime type do arquivo
        ...
        public IActionResult RelatorioPdf()
        {
            try
            {
                ...
                var categoriasReportService = new CategoriasReportService();
                var relatorio = categoriasReportService.GerarRelatorioPdf(categoria);
        ...
        private IdentityViewModel UsuarioAutenticado
        {
            get
            {
                //capturando os dados do usuário autenticado (Cookie)
